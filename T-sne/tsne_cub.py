...
if __name__ == '__main__':
    ...
    if args.pretrain:
        #path = 'save/' + args.model + args.backbone + 'cub' + '/'
        #path = '/home/rugu/桌面/CUB_rotation_多层_0.8eq/'
        path = '/home/rugu/桌面/model/cub/rotation/'
        path ='C:/Users/13624/Desktop/few-shot - som/save/model/cub/rotation/'
    ...
    #base_mean
    classes_count = np.zeros([100,])
    feature_matrix = np.zeros([100, 640])
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_loader):
            ...
            temp = temp[::5, :]
            real = np.mean(temp, axis=0)
            real = np.expand_dims(real, axis=0)
            temp = np.concatenate((real, temp), axis=0)
            ...
            plt.scatter(result[0, 0], result[0, 1], marker="^", s=120,
                        c=plt.cm.Set1(1.0 / 8.0), alpha=1, label='真实原型')

            plt.scatter(result[1, 0], result[1, 1], marker="p", s=120,
                        c=plt.cm.Set1(2.0 / 8.0), alpha=1, label='矫正原型')

            plt.scatter(result[2, 0], result[2, 1], marker="s", s=120,
                        c=plt.cm.Set1(3.0 / 8.0), alpha=1, label='原型')

            plt.scatter(result[3:, 0], result[3:, 1], marker='o', s=10,
                        c=plt.cm.Set1(4.0 / 8.0), alpha=1,label='样本特征')
                    # elif i>=5 and i<10:
            ...
            plt.title("")
            plt.savefig('C:/Users/13624/Desktop/few-shot - som/save/model/mini/rotation/cub.jpeg', format='jpeg')
            plt.show()
            break
