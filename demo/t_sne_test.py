...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_loader):
            ...
            label = np.array([0, 1, 2, 3, 4] * 200)
            label_shot = np.array([0, 1, 2, 3, 4])

            tsne = TSNE(n_components=2, init='pca', random_state=0)

            result = tsne.fit_transform(data_query)
            shot = tsne.fit_transform(shot)
            shot_aug = tsne.fit_transform(data_shot)
            temp = np.concatenate((result,shot),axis=0)
            x_min, x_max = np.min(temp, 0), np.max(temp, 0)
            print("x_min",x_min,x_max)
            result = (result - x_min) / (x_max - x_min)
            shot = (shot - x_min) / (x_max - x_min)
            shot_aug = (shot_aug - x_min) / (x_max - x_min)
            print("shot:",shot)
            print("shot_aug:",shot_aug)
            print("query:",result[:5,:])
            fig = plt.figure(dpi=200)
            for i in range(shot.shape[0]):
                plt.scatter(shot[i, 0], shot[i, 1], marker="^", s=20,
